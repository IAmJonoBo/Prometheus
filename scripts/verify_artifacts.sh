...
main() {
    printf "${BLUE}╔═══════════════════════════════════════════════════════════╗${NC}\n"
    printf "${BLUE}║  Prometheus Artifact Verification (PR #90 Prevention)    ║${NC}\n"
    printf "${BLUE}╚═══════════════════════════════════════════════════════════╝${NC}\n"
    ...
    if [[ ${ERRORS} -gt 0 ]]; then
        ...
        printf "${YELLOW}These issues must be fixed before deploying to air-gapped environments.${NC}\n"
        return 1
    elif [[ ${WARNINGS} -gt 0 ]]; then
        printf "${YELLOW}⚠ Validation completed with %d warning(s)${NC}\n" "${WARNINGS}"
