        ...
        for error in errors:
            ...
            if "manifest" in error_str:
                actions.append(RemediationAction(
                    ...
                    command=["python", "-m", "chiron", "package", "offline", "--manifest-only"],
                    confidence=0.7,
                    auto_apply=False,
                ))
    ...
    def remediate_configuration_drift(
        ...
        if drift_count > 0:
            actions.append(RemediationAction(
                ...
                command=["python", "-m", "chiron", "deps", "sync", "--apply", "--force"],
                confidence=0.85,
                auto_apply=True,
                rollback_command=["git", "checkout", "pyproject.toml", "poetry.lock"],
            ))
    ...
    def _apply_actions(
        ...
        for action in actions:
            ...
            try:
                ...
                elif action.action_type == "file_edit" and action.file_path and action.file_content:
                    logger.info(f"Applying: {action.description}")
